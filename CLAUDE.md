# Large Language Monkeys - Agent Guidelines

## Environment Setup
- Two conda environments required:
  - `llmonk-minif2f`: Python 3.9.19 (for MiniF2F evaluation)
  - `llmonk`: Python 3.11.8 (for everything else)

## Running Commands
- Always activate the appropriate conda environment first:
  ```bash
  conda activate llmonk-minif2f && python llmonk/generate/minif2f.py ...
  ```

## Test Commands
- Run single unit test: `conda activate llmonk-minif2f && python llmonk/tests/minif2f.py`
- Run MiniF2F evaluation: `CONTAINER=native CACHE_DIR=$SCRATCH_DIR/cache TMP_DIR=$SCRATCH_DIR/lean_tmp python llmonk/evaluate/minif2f.py samples_dir=... save_dir=...`
- See `commands.md` for more detailed examples of generation and evaluation

## Code Style
- Use YAML for data storage/retrieval with `utils.py` functions
- Type hints required for function parameters
- Follow PEP8 naming conventions (snake_case for functions/variables)
- Use dataclasses/Config classes for configuration management
- Handle errors with clear error messages and appropriate exception handling
- For API calls, use proper error handling with retries/timeouts
- Organize code by dataset (separate module for each dataset's generation/evaluation)
...
- [Structured Output](https://inspect.aisi.org.uk/structured.html.md) explains how to constrain model output to a particular JSON schema.
...
- [Agent API](https://inspect.aisi.org.uk/agents-api.html.md): This article describes advanced Inspect APIs available for creating evaluations with agents.
- [Agent Bridge](agent-bridge.qmd): Facility for integrating agents from 3rd party frameworks like AutoGen or LangChain.
- [Human Agent](https://inspect.aisi.org.uk/human-agent.html.md): This article describes the `human_agent()` solver which enables human baselining for computing tasks.
- [Approval](https://inspect.aisi.org.uk/approval.html.md): Approvals enable you to create fine-grained policies for approving tool calls made by models.
...
- [Typing](https://inspect.aisi.org.uk/typing.html.md): Provides guideance on using static type checking with Inspect, including creating typed interfaces to untyped storage (i.e. sample metadata and store).
